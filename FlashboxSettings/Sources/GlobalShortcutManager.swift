...
/// that the user can press from anywhere to toggle Flashbox panels.
@MainActor
public final class GlobalShortcutManager {
    ...
    private func registerCarbonHotKey(key: String, modifiers: NSEvent.ModifierFlags) {
        guard let keyCode = virtualKeyCodes[key.uppercased()] else {
            NSLog("[FlashboxSettings] Carbon: unknown key '%@'", key)
        ...
        guard registerError == noErr, let hotKeyRef = ref else {
            NSLog(
                "[FlashboxSettings] RegisterEventHotKey failed (err=%d), falling back to global monitor",
                registerError
            )
            fallbackToGlobalMonitor(key: key, modifiers: modifiers)
            return
    ...
    private func installCarbonEventHandler() {
        ...
        guard let target = GetEventDispatcherTarget() else {
            NSLog("[FlashboxSettings] GetEventDispatcherTarget returned nil")
            return
        }
        ...
        if installError == noErr {
            ...
            NSLog("[FlashboxSettings] InstallEventHandler failed: %d", installError)
        }
    }
    ...
    /// Unique signature for Flashbox hotkeys: "FBHK" (Flashbox HotKey).
